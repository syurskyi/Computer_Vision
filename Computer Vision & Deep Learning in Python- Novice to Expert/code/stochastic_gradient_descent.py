...
def get_batches(X, y, batch_size):
    #loop through the training data and yields the tuples which contains batches of data
...
for epoch in np.arange(0,no_of_epochs):
    ...
    for (batchX, batchY) in get_batches(trainX, trainY, batch_size):
        ...
        #find the loss value and append it to the loss_for_epoch list
    ...
    #find the average loss and add it to the global loss list
    losses_value_avg = np.average(loss_for_epoch)  
    losses_value.append(losses_value_avg)
